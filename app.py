...
Model: TF-IDF + XGBoost (model terbaik hasil perbandingan algoritma)
"""
...
# ---------------------------------------------------------------------------
# Layout: kolom input (kiri, dengan wordcloud di atasnya) & kolom hasil (kanan)
# ---------------------------------------------------------------------------
col_input, col_result = st.columns([1, 1], gap="large")

with col_input:
    st.markdown("**🔤 Gambaran kata kunci kategori prediksi:**")
    if st.session_state.pred_label:
        ...
        st.caption("WordCloud kategori akan muncul di sini setelah kamu melakukan prediksi.")
...
with col_result:
    if st.session_state.pred_label:
        display_text, style = LABEL_INFO.get(
            st.session_state.pred_label, (st.session_state.pred_label, "danger")
        )
        st.markdown(
        ...
        if show_clean:
            st.text_area(
                ...
                value=st.session_state.pred_cleaned,
                height=80,
                disabled=True,
            )
    else:
...
# ---------------------------------------------------------------------------
# Proses prediksi (dieksekusi setelah tombol diklik, lalu rerun agar
# wordcloud di atas kolom input ikut ter-update dengan hasil terbaru)
# ---------------------------------------------------------------------------
if predict_btn:
    if not user_input.strip():
        ...
        if not cleaned.strip():
            ...
            st.rerun()  # rerun langsung supaya wordcloud & hasil konsisten dalam satu tampilan
